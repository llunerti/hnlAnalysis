...
class DsToPhiPi_PhiToMuMu_miniAOD : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
  ...
  private:
    ...
    std::vector<float> *C_Bs_vertex_prob;
    std::vector<float> *C_Bs_mass;
    std::vector<float> *C_Bs_preFit_mass;
    std::vector<float> *C_Bs_px, *C_Bs_py, *C_Bs_pz;
    std::vector<float> *C_Bs_pt;
    std::vector<float> *C_Bs_vertex_x, *C_Bs_vertex_y, *C_Bs_vertex_z;
    std::vector<float> *C_Bs_vertex_xErr, *C_Bs_vertex_yErr, *C_Bs_vertex_zErr;
    std::vector<float> *C_Bs_vertex_sig;
    std::vector<float> *C_Bs_vertex_cos3D;
    std::vector<float> *C_Bs_vertex_cos2D;
  ...
  */
  ...
  for ( unsigned i_mub=0; i_mub<thePATMuonHandle->size(); ++i_mub){
	...
	for (std::vector<pat::PackedCandidate>::const_iterator iTrack1 = thePATTrackHandle->begin(); iTrack1 != thePATTrackHandle->end(); ++iTrack1){
	  ...
	  //   SAVE
	  C_Bs_vertex_prob ->push_back(bs_vtxprob);
	  C_Bs_mass ->push_back(fitted_bs_mass);
	  C_Bs_preFit_mass ->push_back((p4mub + p4pi1 + p4muphi1 + p4muphi2).M());
	  C_Bs_px ->push_back(px_bs);
	  C_Bs_py ->push_back(py_bs);
	  C_Bs_pz ->push_back(pz_bs);
	  C_Bs_pt ->push_back(pt_bs);
	  C_Bs_vertex_x   ->push_back(bs_vx);
	  C_Bs_vertex_y   ->push_back(bs_vy);
	  C_Bs_vertex_z   ->push_back(bs_vz);
	  C_Bs_vertex_xErr->push_back(bs_vxErr);
	  C_Bs_vertex_yErr->push_back(bs_vyErr);
	  C_Bs_vertex_zErr->push_back(bs_vzErr);
	  C_Bs_vertex_sig->push_back(sqrt(bs_vx*bs_vx + bs_vy*bs_vy + bs_vz*bs_vz)/sqrt(bs_vxErr*bs_vxErr + bs_vyErr*bs_vyErr + bs_vzErr*bs_vzErr));
	  C_Bs_vertex_cos3D->push_back(cos3D_bs);
	  C_Bs_vertex_cos2D->push_back(cos2D_bs);
	  ...
	  C_mu_B_px ->push_back(iMuonB->px());
	  C_mu_B_py ->push_back(iMuonB->py());
	  C_mu_B_pz ->push_back(iMuonB->pz());
	  C_mu_B_pt ->push_back(iMuonB->pt());
	  C_mu_B_eta ->push_back(iMuonB->eta());
	  C_mu_B_BS_ips_xy ->push_back(iMuonB->dB(pat::Muon::BS2D)/iMuonB->edB(pat::Muon::BS2D));
	  C_mu_B_BS_ips    ->push_back(iMuonB->dB(pat::Muon::BS3D)/iMuonB->edB(pat::Muon::BS3D));
	  C_mu_B_PV_ips_z  ->push_back(iMuonB->dB(pat::Muon::PVDZ)/iMuonB->edB(pat::Muon::PVDZ));
	  C_mu_B_BS_ip_xy  ->push_back(iMuonB->dB(pat::Muon::BS2D));
	  C_mu_B_BS_ip     ->push_back(iMuonB->dB(pat::Muon::BS3D));
	  C_mu_B_PV_ip_z   ->push_back(iMuonB->dB(pat::Muon::PVDZ));
	  C_mu_B_charge ->push_back(iMuonB->charge());
	  C_mu_B_isSoft   ->push_back(isSoftMuonB? 1: 0);
	  C_mu_B_isLoose  ->push_back(isLooseMuonB? 1: 0);
	  C_mu_B_isMedium ->push_back(isMediumMuonB? 1: 0);
  ...
  C_Bs_vertex_prob->clear();
  C_Bs_mass->clear(); 
  C_Bs_preFit_mass->clear(); 
  C_Bs_px->clear();
  C_Bs_py->clear();
  C_Bs_pz->clear();
  C_Bs_pt->clear();
  ...
  C_Bs_vertex_y->clear();
  C_Bs_vertex_z->clear();
  C_Bs_vertex_xErr->clear();
  C_Bs_vertex_yErr->clear();
  C_Bs_vertex_zErr->clear();
  C_Bs_vertex_sig->clear();
  C_Bs_vertex_cos3D->clear();
  C_Bs_vertex_cos2D->clear();
...
void DsToPhiPi_PhiToMuMu_miniAOD::beginJob()
{
  ...
  wwtree->Branch("C_Bs_vertex_prob", &C_Bs_vertex_prob);
  wwtree->Branch("C_Bs_mass", &C_Bs_mass);
  wwtree->Branch("C_Bs_preFit_mass", &C_Bs_preFit_mass);
  wwtree->Branch("C_Bs_px", &C_Bs_px);
  wwtree->Branch("C_Bs_py", &C_Bs_py);
  wwtree->Branch("C_Bs_pz", &C_Bs_pz);
  wwtree->Branch("C_Bs_pt", &C_Bs_pt);
  wwtree->Branch("C_Bs_vertex_x" , &C_Bs_vertex_x);
  wwtree->Branch("C_Bs_vertex_y" , &C_Bs_vertex_y);
  wwtree->Branch("C_Bs_vertex_z" , &C_Bs_vertex_z);
  wwtree->Branch("C_Bs_vertex_xErr" , &C_Bs_vertex_xErr);
  wwtree->Branch("C_Bs_vertex_yErr" , &C_Bs_vertex_yErr);
  wwtree->Branch("C_Bs_vertex_zErr" , &C_Bs_vertex_zErr);
  wwtree->Branch("C_Bs_vertex_sig" , &C_Bs_vertex_sig);
  wwtree->Branch("C_Bs_vertex_cos3D" , &C_Bs_vertex_cos3D);
  wwtree->Branch("C_Bs_vertex_cos2D" , &C_Bs_vertex_cos2D);
